package io.github.dagonco.sorteo.commons
...
sealed class Either<out L, out R> {
    ...
    fun run(actionLeft: (L) -> Unit = {}, actionRight: (R) -> Unit = {}): Either<L, R> = also { either ->
        either.mapLeft { actionLeft(it) }
        either.map { actionRight(it) }
    }

    suspend fun runSuspend(actionLeft: suspend (L) -> Unit = {}, actionRight: suspend (R) -> Unit = {}): Either<L, R> = also { either ->
        either.mapLeft { actionLeft(it) }
        either.map { actionRight(it) }
    }
}
...
fun <L> Either<L, *>.getLeftOrNull(): L? = fold({ it }, { null })
